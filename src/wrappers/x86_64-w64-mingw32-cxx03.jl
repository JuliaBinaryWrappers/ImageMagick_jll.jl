...
export identify, convert, libwand
...
const identify_splitpath = ["bin", "identify.exe"]
...
const convert_splitpath = ["bin", "convert.exe"]

# This will be filled out by __init__() for all products, as it must be done at runtime
convert_path = ""

# convert-specific global declaration
function convert(f::Function; adjust_PATH::Bool = true, adjust_LIBPATH::Bool = true)
    global PATH, LIBPATH
    env_mapping = Dict{String,String}()
    if adjust_PATH
        if !isempty(get(ENV, "PATH", ""))
...
const libwand_splitpath = ["bin", "libMagickWand-6.Q16-6.dll"]
...
const libwand = "libMagickWand-6.Q16-6.dll"
...
function __init__()
    ...
    global convert_path = normpath(joinpath(artifact_dir, convert_splitpath...))

    push!(PATH_list, dirname(convert_path))
